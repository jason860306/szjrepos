...
public class PoiAroundSearchActivity extends FragmentActivity implements
	...
	private void setUpMap() {
		...
		selectCity.setOnItemSelectedListener(this);// 添加spinner选择框监听事件
	...
	protected void doSearchQuery() {
		...
		currentPage = 0;
		query = new PoiSearch.Query("", "商店", cityCode);// 第一个参数表示搜索字符串，第二个参数表示poi搜索类型，第三个参数表示poi搜索区域（空字符串代表全国）
		...
		if (lp != null) {
			...
			//poiSearch.setBound(new SearchBound(lp, 1000, true));// 设置搜索区域为以lp点为圆心，其周围1000米范围
