package
{
	...
	public class testtcp extends Sprite
	{
		...
		{
//			_timer.stop();
//			_timer.removeEventListener("snd_timer", onSendTimerHandler);
//			_timer = null;
...
//				_socket.addEventListener( OutputProgressEvent.OUTPUT_PROGRESS, onSendData ); //监听数据发送成功事件
		...
		private function onConnect( event:Event ) : void
		{
			...
			_timer = new Timer(5);
			_timer.addEventListener(TimerEvent.TIMER, onSendTimerHandler);
			_timer.start();
		}
		...
		{
			trace("send data..." + _sndCnt);
			_testBuffer.clear();
//			_testBuffer.writeInt(_sndCnt);
			for (var i:uint = 0; i < 1 * 1024 * 1024; ++i)
			{
				_testBuffer.writeByte(i);
			}
//			ba.writeMultiByte("无法解析的外部符号 __imp__closesocket@4，该符号在函数 _wmain 中被引用无法解析的外部符号 __imp__closesocket@4，该符号在函数 _wmain 中被引用无法解析的外部符号 __imp__closesocket@4，该符号在函数 _wmain 中被引用 " + _sndCnt + "\r\n", "gb2312");
			send(_testBuffer);
			++_sndCnt;
		}
